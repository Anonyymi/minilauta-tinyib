...
function insertPost($post) {
	global $dbh;
	$now = time();
	$stm = $dbh->prepare("INSERT INTO " . TINYIB_DBPOSTS . " (parent, timestamp, bumped, ip, name, tripcode, email,   nameblock, subject, message, password,   file, file_hex, file_original, file_size, file_size_formatted, image_width, image_height, thumb, thumb_width, thumb_height, moderated, country_code) " .
		" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
function allThreadsOverboard($boards, $offset = 0, $limit = 10, $moderated_only = true) {
	$threads = array();
	$boards_n = count($boards);
	$queryStr = "SELECT t.* FROM (";
	for($i = 0; $i < $boards_n; $i++) {
		$queryStr .= "SELECT *, '" . $boards[$i] . "' AS board FROM " . $boards[$i] . "_posts WHERE parent = 0" . ($moderated_only ? " AND moderated > 0" : "") . "\n";
		if ($i < $boards_n - 1) {
			$queryStr .= "UNION ALL\n";
		}
	}
	$queryStr .= ") AS t ORDER BY t.bumped DESC LIMIT ? OFFSET ?";
	$results = pdoQuery($queryStr, array($limit, $offset));
	while ($row = $results->fetch()) {
		$threads[] = $row;
	}
	return $threads;
...
function countThreadsOverboard($boards, $moderated_only = true) {
	$boards_n = count($boards);
	$queryStr = "SELECT COUNT(*) FROM (";
	for($i = 0; $i < $boards_n; $i++) {
		$queryStr .= "SELECT id, '" . $boards[$i] . "' AS board FROM " . $boards[$i] . "_posts WHERE parent = 0" . ($moderated_only ? " AND moderated > 0" : "") . "\n";
		if ($i < $boards_n - 1) {
			$queryStr .= "UNION ALL\n";
		}
	}
	$queryStr .= ") AS t";
	$result = pdoQuery($queryStr);
	return (int)$result->fetchColumn();
}
...
function numRepliesToThreadByIDAndBoardID($boardid, $id) {
	$result = pdoQuery("SELECT COUNT(*) FROM " . $boardid . "_posts WHERE parent = ? AND moderated > 0", array($id));
	return (int)$result->fetchColumn();
}
...
function postsInThreadByIDOverboard($boardid, $id, $moderated_only = true) {
	$id = intval($id);
	$posts = array();
	$results = pdoQuery("SELECT *, '" . $boardid . "' AS board FROM " . $boardid . "_posts WHERE (id = ? OR parent = ?)" . ($moderated_only ? " AND moderated > 0" : "") . " ORDER BY id ASC", array($id, $id));
	while ($row = $results->fetch(PDO::FETCH_ASSOC)) {
		$posts[] = $row;
	}
	return $posts;
...
function postsByHex($hex) {
	$posts = array();
	$results = pdoQuery("SELECT * FROM " . TINYIB_DBPOSTS . " WHERE file_hex = ? AND moderated > 0 LIMIT 10", array($hex));
	while ($row = $results->fetch(PDO::FETCH_ASSOC)) {
		$posts[] = $row;
	}
	return $posts;

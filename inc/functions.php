...
function fixLinksInRes($html) {
	$search = array(' href="css/', ' src="js/', ' href="src/', ' href="thumb/', ' href="res/', ' href="imgboard.php', ' href="catalog.html', ' href="loadstyle.php', ' href="favicon.ico', 'src="thumb/', 'src="inc/', 'src="sticky.png', 'src="lock.png', ' action="imgboard.php', ' action="catalog.html');
	$replace = array(' href="../css/', ' src="../js/', ' href="../src/', ' href="../thumb/', ' href="../res/', ' href="../imgboard.php', ' href="../catalog.html', ' href="../loadstyle.php', ' href="../favicon.ico', 'src="../thumb/', 'src="../inc/', 'src="../sticky.png', 'src="../lock.png', ' action="../imgboard.php', ' action="../catalog.html');
...
function checkDuplicateFile($hex) {
	...
	if (count($hexmatches) > TINYIB_DUPLICATES) {
		$hexmatcherrs = '';
		foreach ($hexmatches as $hexmatch) {
			$hexmatcherrs .= $hexmatch['id'] . ': ';
			$hexmatcherrs .= sprintf(__('Duplicate file uploaded. That file has already been posted <a href="%s">here</a>.'), 'res/' . (($hexmatch['parent'] == TINYIB_NEWTHREAD) ? $hexmatch['id'] : $hexmatch['parent']) . '.html#' . $hexmatch['id']);
			$hexmatcherrs .= '<br>';
		}
		fancyDie($hexmatcherrs);
	}
}
...
function createThumbnail($file_location, $thumb_location, $new_w, $new_h, $spoiler) {
	...
	if (TINYIB_THUMBNAIL == 'gd' || (TINYIB_THUMBNAIL == 'ffmpeg' && preg_match("/jpg|jpeg/", $system[0]))) {
		...
		if (preg_match("/png/", $system[0])) {
			...
			if (!imagewebp($dst_img, $thumb_location)) {
				return false;
			}
		} else if (preg_match("/gif/", $system[0])) {
	...
	if (preg_match("/jpg|jpeg/", $system[0])) {
		...
		$src_img = imagecreatefromwebp($thumb_location);
	} else if (preg_match("/gif/", $system[0])) {
	...
	if (preg_match("/png/", $system[0])) {
		...
		if (!imagewebp($src_img, $thumb_location)) {
			return false;
		}
	} else if (preg_match("/gif/", $system[0])) {
...
function processImage($filename) {
	...
	exec("convert -version", $discard, $exit_status);
	if ($exit_status != 0) {
		fancyDie('ImageMagick is not installed, or the convert command is not in the server\'s $PATH.<br>Install ImageMagick.');
	}

	$discard = '';
	$exit_status = 1;
	$filename = escapeshellarg($filename);
	exec("convert $filename -auto-orient $filename", $discard, $exit_status);
}
